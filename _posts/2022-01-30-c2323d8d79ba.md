...
因為前兩個屬性的值（應用程式編號與密鑰）都屬於敏感的資料，因此實務上不會直接把值光明正大地寫在專案裡，避免資安的疑慮。比較好的做法是像圖中那樣，把資料放在環境變數 \(environment variable\)中。說明參考 [這裡]https://martinchiu.github.io/blog/posts/bde35a641b48/)
