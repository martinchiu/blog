...
在撰寫commit 時也有一些工程師間通用的 [規矩](https://wadehuanglearning.blogspot.com/2019/05/commit-commit-commit-why-what-commit.html){:target="_blank"} ，例如每個不同功能（意義）的異動應該獨立成不同的commit，commit 的組成也可以用以下的規範所組成：
